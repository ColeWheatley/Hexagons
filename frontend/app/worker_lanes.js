...
export function workerLaneSizes(hardwareConcurrency = 4) {
    const hardware = Math.max(1, Number(hardwareConcurrency) || 4);
    return {
        geometry: Math.min(6, Math.max(2, hardware)),
        texture: Math.min(MAX_TEXTURE_DECODERS, Math.max(1, hardware - 1)),
    };
}
