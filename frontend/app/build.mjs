...
// Dev-only: recursively copies `sourceRelativePath` into dist, preserving
// its subtree. Only used for powfinder_fixtures/ below, gated behind an
// opt-in env flag -- see copyStaticAssets.
async function copyDirectoryRecursive(sourceRelativePath, targetRelativePath = sourceRelativePath) {
    const sourceDir = path.join(appDir, sourceRelativePath);
    if (!await pathExists(sourceDir)) return 0;
    const files = await walkFiles(sourceDir);
    for (const file of files) {
        const relative = toPosix(path.relative(sourceDir, file));
        await writeFile(`${targetRelativePath}/${relative}`, await fs.readFile(file));
    }
    return files.length;
}
...
async function copyStaticAssets() {
    ...
    // Dev-only: bundle the local synthetic PowFinder sidecar season
    // (powfinder_fixtures/, gitignored, generated by
    // `node scripts/make_sidecar_fixtures.mjs` -- design doc §6 P0.1) into
    // dist so a local `npm run build` can exercise the full sidecar
    // fetch/cache/self-disable path end-to-end without a real backend.
    // Strictly opt-in and never in a real release build: the fixture season
    // is hundreds of MB and is not a deploy artifact (see DEPLOY.md, which
    // syncs only the real backend's sidecar output to S3).
    if (process.env.HEXAGONS_INCLUDE_POWFINDER_FIXTURES === '1') {
        const copied = await copyDirectoryRecursive('powfinder_fixtures');
        if (copied === 0) {
            console.warn('  HEXAGONS_INCLUDE_POWFINDER_FIXTURES=1 but powfinder_fixtures/ is missing or empty -- run scripts/make_sidecar_fixtures.mjs first.');
        } else {
            console.log(`  dev flag HEXAGONS_INCLUDE_POWFINDER_FIXTURES=1: copied ${copied} powfinder_fixtures file(s)`);
        }
    }
}
