...
`scripts/release_browser_gates.sh` makes three orbit trials and accepts the median of loader hidden / ready <= 15 s, visible textured coverage (TTFTF) <= 30 s, and orbit p95 <= 100 ms. It captures 320, 390, 768 and 1280 px viewport screenshots as review artifacts. These are intentionally not pixel-goldens: GPU output, font rasterization and baked data make pixel equality flaky.
