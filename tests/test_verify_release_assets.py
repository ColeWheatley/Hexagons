...
class ReleaseAssetVerifierTest(unittest.TestCase):
    ...
    def test_counts_every_valid_referenced_file(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report["by_extension"][".ktx2"]["count"], 1)
    ...
    def test_rejects_empty_and_duplicate_asset_sets(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            payload["tiles"] = []
            payload["texture_pages"]["pages"] = []
            manifest.write_text(json.dumps(payload))
            self.assertIn("no release assets", verifier.verify_assets(manifest, app)["errors"][0])
