...
for (const budget of [2, 3, 4, 6]) {
    const sizes = lanes.workerLaneSizes(budget);
    assert.ok(sizes.geometry >= 1 && sizes.geometry <= 4);
    assert.ok(sizes.texture >= 1 && sizes.texture <= 2);
    assert.equal(sizes.geometry + sizes.texture, budget);
}
...
assert.match(main, /Math\.min\(maxConcurrent, this\.capabilityProfile\.maxTextureJobs\)/);
